...
CREATE TABLE [SubjectClass] (
	...
	UNIQUE (subject_id, class_id, semester_id)
        ...
        public const string DBQ_GET_ACCOUNT_DETAILS_LOG_IN = "SELECT * FROM [Accounts] WHERE [account_name] = @name AND [account_password] = @password",
                            ...
                            DBQ_GET_ALL_SEMESTERS = "SELECT [semester_id] [ID], ('Term:' || [current_term] || ', ' || [starting_school_year] || '-' || [ending_school_year] || ', ' || [Classes].[class_name]) AS [Details] FROM [Semester] INNER JOIN [Classes] ON [Semester].[class_id] = [Classes].[class_id] WHERE [Semester].[account_id] = @account_id ORDER BY [starting_school_year], [ending_school_year] ASC",
